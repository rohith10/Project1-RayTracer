...
__device__ glm::vec3 calcShade (interceptInfo theRightIntercept, glm::vec3 lightVec, glm::vec3 eye, ray castRay, material* textureArray, float ka, float ks, float kd, int materialid)
{
	...
	if (theRightIntercept.interceptVal > 0)
	{
		...
		shadedColour += multiplyVV (textureArray [materialid].color, intrPoint);		// shadedColour will have diffuse shaded colour. 
		...
		shadedColour += (textureArray [materialid].color * ks * pow (interceptValue, theRightIntercept.intrMaterial.specularExponent));
	}
...
//  }
 // __syncthreads ();
  ...
  if((x<=resolution.x && y<=resolution.y))
  {
	...
	shadedColour += calcShade (theRightIntercept, lightVec, cam.position, castRay, textureArray, ka, ks, kd, light.materialid);
	...
	lightVec = glm::normalize (lightPosition - (castRay.origin + (castRay.direction*theRightIntercept.interceptVal)));
	if (hasReflective)
		shadedColour = ((shadedColour * (float)0.92) + (calcShade (theRightIntercept, lightVec, cam.position, castRay, textureArray, ka, ks, kd, light.materialid) * (float)0.08));
...
__device__ bool isShadowRayBlocked (ray r, glm::vec3 lightPos, staticGeom *geomsList, sceneInfo objectCountInfo)
{
	...
	for (int i = 0; i < objectCountInfo.nCubes; ++i)
	{
		...
		interceptValue = boxIntersectionTest(geomsList, i, r, intrPoint, intrNormal);
		if (interceptValue > 0)
		{
			if (interceptValue < min)
				min = interceptValue;
	...
	{
		staticGeom currentGeom = geomsList [i];
		interceptValue = sphereIntersectionTest(geomsList,i, r, intrPoint, intrNormal);
		if (interceptValue > 0)
		{
			if (interceptValue < min)
				min = interceptValue;
